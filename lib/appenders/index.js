...
// separately and painstakingly require each appender so that webpack knows to grab them
...
const logLevelFilter=require('./logLevelFilter');
...
if(typeof window === 'undefined'){ // these cause errors on the browser - don't load them there
  const noLogFilter=require('./noLogFilter');
  coreAppenders.set('noLogFilter',noLogFilter);
  const file=require('./file');
  coreAppenders.set('file',file);
  const dateFile=require('./dateFile');
