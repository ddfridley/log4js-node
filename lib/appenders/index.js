...
// but if any appenders weren't found - keep going
const coreAppenders = new Map();
var coreAppenderNames;
if(typeof window==='undefined') 
  coreAppenderNames=['console','stdout','stderr','logLevelFilter', 'categoryFilter','noLogFilter','file','dateFile']
else 
  coreAppenderNames=['console','stdout','stderr']
coreAppenderNames.map(appender=>{
  try{
    coreAppenders.set(appender, require('./'+appender));
  }
  catch(err){
    debug('Loading coreAppender did not succeed', file, 'skipping');
  }
})
